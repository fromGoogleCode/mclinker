...
void ObjectLinker::addSymbolToOutput(ResolveInfo& pInfo, Module& pModule) {
  ...
  // the merged one
  if (out_sym->hasFragRef() && (m_LDBackend.isMergeStringSection(
                                    out_sym
                                        ->fragRef()
                                        ->frag()
                                        ->getParent()
                                        ->getSection()))) {
    assert(out_sym->fragRef()->frag()->getParent()->getSection()
               .hasMergeString());
    MergeString* ms = out_sym->fragRef()->frag()->getParent()->getSection()
                          .getMergeString();
  ...
  for (input = m_pModule->obj_begin(); input != inEnd; ++input) {
    ...
    for (rs = (*input)->context()->relocSectBegin(); rs != rsEnd; ++rs) {
      ...
      for (reloc = (*rs)->getRelocData()->begin(); reloc != rEnd; ++reloc) {
        ...
        // target symbol defined in merge string section
        if (info->outSymbol()->hasFragRef()) {
          LDSection& sym_sect = info->outSymbol()->fragRef()->frag()
                                    ->getParent()->getSection();
          if (m_LDBackend.isMergeStringSection(sym_sect)) {
            m_LDBackend.getRelocator()->applyRelocationForMergeString(
                *relocation, *sym_sect.getMergeString());
            continue;
          }
        }
